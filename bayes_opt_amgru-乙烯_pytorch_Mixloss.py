...
class AMGRU(nn.Module):
    def __init__(self, input_size, hidden_size):
        super().__init__()
        self.gru = nn.GRU(input_size, hidden_size, batch_first=True)  # 修改为 batch_first=True
...
class ModelTrainer:
    """模型训练器"""
    def __init__(self, device: torch.device, logger: Logger,delta: float):
        self.device = device
        self.criterion = DeltaMixLoss(delta)
        self.logger = logger
    

    def train(self, model: AMGRU, train_loader: DataLoader, 
    ...
    def evaluate(self, model: AMGRU, test_x: torch.Tensor, 
...
class Train_OPT:
    ...
    def train_test(self, delta: float, hidden: float, epoch: float, batch: float) -> float:
        ...
        model = AMGRU(train_x.shape[2], int(hidden))
